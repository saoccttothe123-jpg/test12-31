...
module.exports.run = async ({ message, args, lang }) => {
	...
	if (!message.member.permissions.has(PermissionsBitField.Flags.KickMembers) && userId !== message.client.user.id) {
	...
	if (userId === message.user.id) {
		return message.reply({ content: "Bạn không thể tự kick chính mình.", ephemeral: true });
	}
	...
	try {
		await member.user.send(`Bạn đã bị đuổi khỏi ${message.guild.name} vì: ${reason}`);
	} catch (error) {
		console.error(`Không thể gửi tin nhắn trực tiếp cho ${member.user.tag}:`, error);
	}

	await member.kick(reason);
	message.reply({ content: `Đã đuổi ${member.user.tag} vì: ${reason}` });
};
